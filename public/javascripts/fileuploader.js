...
qq.FileUploader = function(o){
    this._options = {
        ...
        template: '<div class="qq-uploader">' +
            '<div class="qq-upload-drop-area"><span style="font-size: 20px;top:120px;">请把APK文件或者APK文件压缩成的zip包拖拽到区域内上传</span></div>' +
            '<div class="qq-upload-button" style="display:none">选择文件上传</div>' +
        ...
        fileTemplate: '<li>' +
            ...
            '<button   class="qq-upload-handle" style="display: none; background-color:#C8C8C8;font-size: large;border-radius: 12px 12px 12px 12px;">处理</button>'+
            '<button   class="qq-upload-cancelUpload" style="display: none;background-color:#C8C8C8;font-size: large;border-radius: 12px 12px 12px 12px;">取消</button>'+
            '<button   class="qq-upload-back" style="display: none; background-color:#C8C8C8;font-size: large;border-radius: 12px 12px 12px 12px;">重新上传</button>'+
            '</li>',
        ...
        classes: {
            ...
            handle:'qq-upload-handle',
            back:'qq-upload-back',
            cancelUpload:'qq-upload-cancelUpload',
//            return:"qq-upload-return",
        ...
        messages: {
            ...
            typeError: "{file} 类型不正确！！！请上传 {extensions}类型的文件！",
    ...
    _createUploadHandler: function(){
        ...
        var handler = new qq[handlerClass]({
            ...
            onProgress: function(id, fileName, loaded, total){
                ...
                document.getElementById("returnIndex").style.display="none";
            ...
            onComplete: function(id, fileName, result){
                ...
                if (result.success){
                    self._getElement(item, 'handle').style.display ="block";
                    self._getElement(item, 'handle').style.display = 'inline';
                    self._getElement(item, 'handle').onclick=function () { location.href ="/handle/"+fileName };
                    self._getElement(item, 'cancelUpload').style.display ="block";
                    self._getElement(item, 'cancelUpload').style.display = 'inline';
                    self._getElement(item, 'cancelUpload').onclick=function () { location.href ="/delete/"+fileName };
                    document.getElementById("returnIndex").style.display="none";
                    qq.addClass(item, self._classes.success);
                } else {
                    document.getElementById("returnIndex").style.display="block";
                    self._getElement(item, 'back').style.display ="block";
                    self._getElement(item, 'back').style.display = 'inline';
                    self._getElement(item, 'back').onclick=function () {
                        var test = document.getElementById("dropbox");
...
//                this._uploadFile(input);
    ...
    _uploadFileList: function(files){
        ...
        if (valid){
            ...
            if(i>1){
                alert("一次只能上传一个文件！");
                 this._getElement('drop').style.display = 'block';
                var test = document.getElementById("dropbox");
                var children = test.childNodes;
                for(i=0;i<children.length;i++){
                    test.removeChild(children[i]);
                ...
                if(this._getElement('list').children.length>0){
                    alert("每次只允许上传一个文件！");
                }
                else{
    ...
    _updateProgress: function(id, loaded, total){
        ...
        if (loaded != total){
            ...
            text ='100%     '+ this._formatSize(total);
        }
...
//        console.log(file);
        ...
        // build query string
        var queryString = '?fileName=' + encodeURIComponent(name)+'&fileSize='+encodeURIComponent(size);
